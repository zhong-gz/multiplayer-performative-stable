...
for p in [0]:  #,1,2,3,4
  price_index = p
  data_file_path = f'./data_ride_share/{price_index*5+10}_data.npy'
  # set up the game
  ...
  if 1:
    ...
    for num_exper in range(num_experiments):
      ...
      # run all cases
      dic_data = []
      dic_data.append(ddgame.runRR(gamma = gamma,price_index=price_index,BATCH=BATCH,MAXITER=MAXITER, perform_rr=[True,True],tot_rev=tot_rev))
  ...
  for model in models:
    ...
    color = model_colors[model]
    alpha = company_styles[company]['alpha']
    plt.plot(avg_data[key1]+avg_data[key2], label=f'{model}',lw=lw,
